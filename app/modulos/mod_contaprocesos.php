                        ...
                        {$msg='ERR. No se ha definido el tipo de comprobante, ir a parámetros';}
                            ...
                            $nomComprobante = nombreComprobante($empresa, $comprobante);
                             $msg=$periodoFac.'||'.$nuevoPeriodo.'||'.$fecCorte.'||'.$comprobante .'||'.
                                  ...
                                  '||'. $RecargoPorc .'||'. $RecargoPesos .'||'. $RecargoDias .'||'. $FactorRedondeo .'||'.$PeriCierreFactura;
                        }             
                    echo $msg;
...
//$empresa+"||"+prop+"||"+inmu+"||"+forma+"||"+valor+"||"+referencia+"||"+fecha;   
            ...
            while($row = mysqli_fetch_assoc($resultado)) { 
                ...
                if ($valor >= 0){
                    ...
                    grabaContapagos($rec[0], $id, $rec[6], $rec[3], $pago, 'Paga Intereses de mora', $consecReciCaja ,$inmueble);
                    ...
                    if($valor >= 0){
                        ...
                        $resul = mysqli_query($con, $sql);                      
                        grabaContapagos($rec[0], $idFac, $rec[6], $rec[3], $pago, $detalle, $consecReciCaja, $inmueble );
                    }
                    else{
                        ...
                        $resul = mysqli_query($con, $sql); 
                        grabaContapagos($rec[0], $idFac, $rec[6], $rec[3], $valor, $detalle, $consecReciCaja, $inmueble );
                    }
                }
                else{
                    // No le alcanza para pagar toda la mora
                    grabaContapagos($rec[0], $id, $rec[6], $rec[3], $saldo, 'Anticipo no cubre toda la mora', $consecReciCaja, $inmueble ); 
            ...
            while($row = mysqli_fetch_assoc($resultado)) {
               ...
               // 
                ...
                if ($valor >= 0){                  
                    ...
                    $resul = mysqli_query($con, $sql); 
                    grabaContapagos($rec[0], $id, $rec[6], $rec[3], $pago, $detalle, $consecReciCaja, $inmueble);
                    $saldo = $valor;
                } 
                else{
                  ...
                  $sql = "UPDATE contafactura SET facturasaldo = ".$valor .",  facturaTipo='A'  WHERE facturaid = " . $id;
                    $resul = mysqli_query($con, $sql);   
                    grabaContapagos($rec[0], $id, $rec[6], $rec[3], $valor, $detalle, $consecReciCaja, $inmueble );
                    $saldo= 0;
                }
            }
            if($saldo > 0){
                grabaContapagos($rec[0], 0, $rec[6], $rec[3], $saldo, 'Anticipo '.$row['facturadetalle'], $consecReciCaja, $inmueble );
                grabaAnticipo($rec[0], $consecReciCaja, $inmueble, $saldo, $rec[6], 'Anticipo Sobrante de pagos', $propie); 
            }
 ...
 //$empresa+"||"+prop+"||"+inmu+"||"+forma+"||"+valor+"||"+referencia+"||"+fecha;   
    function grabaContapagos($em, $id, $fch, $tp, $pago, $ref, $cons, $inmu ){
        global $objClase;
        $con = $objClase->conectar();  
        $sql="INSERT INTO contapagos (pagosempresa, pagosfacturaid, pagosfecha, ".
        "pagostipo, pagosvalor, pagosreferencia, pagosNrReciCaja, pagosinmueble) " .
        " VALUES (" .$em . "," . $id . ", '" . $fch . "', '" . $tp . "', ".
        $pago . ", '" . $ref . "', '" . $cons . "',".$inmu.")";
        $resul = mysqli_query($con, $sql);
    }
